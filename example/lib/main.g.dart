...
class UpdateDsEvent extends BaseEvent {
  final List<Map<String, dynamic>> ds;
  const UpdateDsEvent({required this.ds});

  @override
  List<Object?> get props => [ds];
}

class UpdateItemsEvent extends BaseEvent {
  final List<Map<String, dynamic>> items;
  const UpdateItemsEvent({required this.items});
...
class BaseState extends Equatable {
  ...
  static void registerEvents(BaseBloc bloc) {
    ...
    bloc.on<UpdateDsEvent>((event, emit) {
      emit(bloc.state.copyWith(ds: event.ds));
    });

    bloc.on<UpdateItemsEvent>((event, emit) {
      emit(bloc.state.copyWith(items: event.items));
    });
  }
...
extension BaseBlocContextExtension on BuildContext {
  void setBaseBlocState(
    ...
    if (ds != null) {
      myBloc.add(UpdateDsEvent(ds: ds));
    }

    if (items != null) {
      myBloc.add(UpdateItemsEvent(items: items));
    }
  }
}
